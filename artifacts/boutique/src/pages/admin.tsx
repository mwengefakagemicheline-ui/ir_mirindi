...
const productSchema = z.object({
  ...
  categoryId: z.coerce.number().min(1, "Requis"),
...
export function Admin() {
  ...
  const openCreate = () => {
    setEditingId(null);
    reset({ name: "", price: 0, categoryId: 0, stock: 10, description: "", imageUrl: "", brand: "" });
    setIsModalOpen(true);
  };
  ...
  const onSubmit = async (data: ProductForm) => {
    try {
      const payload = {
        ...data,
        imageUrl: data.imageUrl || undefined, // Send undefined if empty
      };
  ...
  const handleDelete = async (id: number) => {
    if (confirm("ÃŠtes-vous sûr de vouloir supprimer ce produit ?")) {
      try {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-display font-medium text-zinc-900">Administration</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-zinc-100">
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-zinc-500">Chargement...</td>
                </tr>
                ...
                <tr key={product.id} className="hover:bg-zinc-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button onClick={() => openEdit(product)} className="p-2 text-zinc-400 hover:text-zinc-900 transition-colors bg-white rounded-md shadow-sm border border-zinc-200">
                      <Edit2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(product.id)} className="p-2 text-zinc-400 hover:text-red-500 transition-colors bg-white rounded-md shadow-sm border border-zinc-200">
      ...
      {/* Modal - keeping it simple and raw for guaranteed execution */}
      {isModalOpen && (
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
              <form id="product-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-zinc-700 mb-1.5">Prix (â‚¬)</label>
